...
const LINK_RE = /(https?:\/\/|www\.|<a\s|\[url|\b[a-z0-9-]+\.(?:com|net|org|ru|xyz|top|info|biz|click|link|shop)\b)/gi;
...
function Contact() {
  ...
  const validate = () => {
    ...
    else if (trimmedMessage.length > 2000) next.message = 'Mensagem muito longa (máximo 2000 caracteres).';
    else if (countLinks(trimmedMessage) >= 2) next.message = 'Mensagem com links demais.';
    else if (REPEAT_RE.test(trimmedMessage)) next.message = 'Mensagem parece conter texto repetido.';
    ...
    <>
      ...
      <section style={{ background: 'var(--accent-go)', padding: 'clamp(60px,8vw,100px) 0', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ maxWidth: 'var(--container)', margin: '0 auto', padding: '0 var(--gutter)', position: 'relative', zIndex: 1 }}>
          <div className="cta-grid" style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: 'clamp(40px,6vw,88px)', alignItems: 'center', textAlign: 'left' }}>
            ...
            <div className="cta-photo" style={{ position: 'relative', width: 'clamp(216px, 24vw, 308px)', justifySelf: 'end', flexShrink: 0 }}>
              <div style={{ position: 'relative', aspectRatio: '4/5', borderRadius: 'var(--radius-lg)', overflow: 'hidden', boxShadow: '0 24px 56px rgba(0,0,0,0.22)' }}>
                <img src={ctaImg} alt="Eliana Lino sorrindo durante uma conversa"
                  style={{ width: '100%', height: '100%', objectFit: 'cover', objectPosition: '50% 30%', filter: 'brightness(0.97) contrast(1.02) saturate(0.92)' }} />
              </div>
              <div style={{ position: 'absolute', bottom: -14, right: -14, width: 96, height: 96, border: '1px solid rgba(255,255,255,0.28)', borderRadius: 'var(--radius-sm)', zIndex: -1 }} />
              <div style={{ position: 'absolute', top: -14, left: -14, width: 60, height: 60, border: '1px solid rgba(255,255,255,0.2)', borderRadius: 'var(--radius-sm)', zIndex: -1 }} />
            </div>
          </div>
        </div>
      </section>
          ...
          <div style={{ opacity: inView ? 1 : 0, transform: inView ? 'translateX(0)' : 'translateX(32px)', transition: 'opacity 1s ease .2s, transform 1s var(--ease-out-expo) .2s' }}>
            {sent ? (
              <div className="animate-fade-in-up" style={{ height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: '60px 32px', background: 'var(--color-olive-100)', borderRadius: 'var(--radius-lg)', border: '1px solid var(--color-olive-200)' }}>
                ...
                <h3 style={{ fontFamily: 'var(--font-serif)', fontSize: '1.6rem', fontWeight: 400, color: 'var(--text-strong)', marginBottom: 12 }}>Tudo pronto!</h3>
                <p style={{ fontFamily: 'var(--font-sans)', fontSize: '0.9rem', fontWeight: 300, color: 'var(--text-muted)', lineHeight: 1.7, marginBottom: 20 }}>
                  Abri o WhatsApp com sua mensagem já escrita. É só tocar em enviar — retorno em até 24 horas.
                </p>
                {waUrl && (
                  <a href={waUrl} target="_blank" rel="noopener noreferrer"
                    style={{ display: 'inline-flex', alignItems: 'center', gap: 8, fontFamily: 'var(--font-sans)', fontSize: '0.8rem', fontWeight: 500, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--color-olive-600)', textDecoration: 'none', borderBottom: '1px solid var(--color-olive-300)', paddingBottom: 4 }}>
              ...
              <form onSubmit={submit} noValidate style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                <div className="form-row" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                  <Input placeholder="Seu nome completo" autoComplete="name" maxLength={80} value={name} error={errors.name} onChange={(e: any) => setName(e.target.value)} />
                  <Input type="email" placeholder="Seu e-mail (opcional)" autoComplete="email" maxLength={120} value={email} error={errors.email} onChange={(e: any) => setEmail(e.target.value)} />
                </div>
                <Input type="tel" placeholder="Telefone (opcional)" autoComplete="tel" value={phone} error={errors.phone} onChange={(e: any) => setPhone(formatPhone(e.target.value))} maxLength={15} />
                <Input as="select" value={subject} error={errors.subject} onChange={(e: any) => setSubject(e.target.value)} options={SUBJECTS} />
                <Input as="textarea" rows={5} placeholder="Conte um pouco sobre o que você está buscando…" maxLength={2000} value={message} error={errors.message} onChange={(e: any) => setMessage(e.target.value)} />

                <Button type="submit" variant="primary" size="lg" tone="light" style={{ alignSelf: 'flex-start' }}>
                  Enviar pelo WhatsApp
                </Button>
                <p style={{ fontFamily: 'var(--font-sans)', fontSize: '0.75rem', fontWeight: 300, color: 'var(--text-muted)', margin: 0 }}>
