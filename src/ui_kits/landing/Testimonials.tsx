...
function Testimonials() {
  const { SectionHeading, IconButton, Avatar, StatCard } = window.ElianaLinoDesignSystem_6994f2;
  const { ChevronLeft, ChevronRight, Quote } = window.Icons;
  ...
  return (
    <section
      id="depoimentos"
      ref={ref}
      style={{ background: 'var(--bg-subtle)', padding: 'var(--section-y) 0', overflow: 'hidden' }}
    >
      <div style={{ maxWidth: 'var(--container)', margin: '0 auto', padding: '0 var(--gutter)' }}>
        ...
        >
          <SectionHeading eyebrow="Depoimentos">
            Histórias de{' '}
            <em style={{ color: 'var(--accent)', fontStyle: 'italic' }}>transformação</em>
          </SectionHeading>
        </div>
        ...
        >
          <div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
              <IconButton aria-label="Depoimento anterior" onClick={prev}>
                <ChevronLeft size={18} />
              </IconButton>
                ...
                {TESTIMONIALS.map((_, i) => (
                  <button
                    ...
                    style={{
                      ...
                      padding: 0,
                      background:
                        i === active ? 'var(--color-terra-400)' : 'var(--color-terra-200)',
                      transition: 'all .35s ease',
                    }}
                  />
                ))}
              </div>
              <IconButton aria-label="Próximo depoimento" onClick={next}>
